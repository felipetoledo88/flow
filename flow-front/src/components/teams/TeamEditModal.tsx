...
import { Team, UpdateTeamDto, User, WEEK_DAYS } from '@/types/team';
...
const TeamEditModal: React.FC<TeamEditModalProps> = ({
  ...
  useEffect(() => {
    if (team && managers.length > 0) {
      const directorId = team.director ? team.director.id.toString() : '';
      setFormData(prev => ({
        ...prev,
        director: directorId,
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        error?.response?.data?.message || error?.message || 'Erro ao atualizar equipe';
